...
Hash: `ad69c0c69ba9` · LOC: 1 · Main guard: false
...
## Classes
- `DartCandidate` (L22): Potential dart detection\n- `DartImpact` (L35): Confirmed dart impact\n- `DartDetectorConfig` (L46): Dart detector configuration\n- `DartImpactDetector` (L149): Dart impact detector with temporal confirmation and cooldown.\n- `FieldMapperConfig` (L463): Field mapper configuration\n- `FieldMapper` (L482): Maps pixel coordinates to dartboard scores.

## Functions
- `apply_detector_preset()` (L142)\n- `__init__()` (L152)\n- `detect_dart()` (L166): Detect dart impact with temporal confirmation and cooldown.\n- `_is_in_cooldown()` (L232): Check if position is in cooldown region.\n- `_preprocess_motion_mask()` (L248): Reduce noise in motion mask before contour extraction.\n- `_find_dart_shapes()` (L267): Find dart-like objects in motion mask\n- `_is_same_position()` (L432): Check if two candidates are at same position\n- `_reset_tracking()` (L447): Reset temporal tracking\n- `get_confirmed_impacts()` (L452): Get all confirmed dart impacts\n- `clear_impacts()` (L456): Clear confirmed impacts (e.g., new game round)\n- `__post_init__()` (L476)\n- `__init__()` (L489)\n- `point_to_score()` (L495): Convert pixel coordinates to dartboard score.

## Intra-module calls (heuristic)
Canny, DartCandidate, DartDetectorConfig, DartImpact, FieldMapperConfig, GaussianBlur, _find_dart_shapes, _is_in_cooldown, _is_same_position, _preprocess_motion_mask, _reset_tracking, abs, append, arcLength, arctan2, boundingRect, clear, clip, contourArea, convexHull, copy, count_nonzero, cvtColor, degrees, deque, dict, findContours, float, fromiter, get, getLogger, info, int, len, lower, max, moments, nonzero, replace, sort, sqrt, threshold
