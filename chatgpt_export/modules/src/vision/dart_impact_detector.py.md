# Module: `src\vision\dart_impact_detector.py`
Hash: `c752eb00a515` · LOC: 1 · Main guard: false
...
## Classes
- `DartCandidate` (L23): Potential dart detection\n- `DartImpact` (L36): Confirmed dart impact\n- `DartDetectorConfig` (L51): Dart detector configuration\n- `DartImpactDetector` (L190): Dart impact detector with temporal confirmation and cooldown.\n- `FieldMapperConfig` (L759): Field mapper configuration\n- `FieldMapper` (L778): Maps pixel coordinates to dartboard scores.

## Functions
- `apply_detector_preset()` (L183)\n- `__init__()` (L193)\n- `_preprocess_motion_mask()` (L208): Adaptive binarization + morphology for stable motion contours.\n- `detect_dart()` (L239): Detect dart impact with temporal confirmation and cooldown.\n- `_is_in_cooldown()` (L360): Check if position is in cooldown region.\n- `_find_dart_shapes()` (L377): Find dart-like objects in motion mask\n- `_refine_impact_in_roi()` (L577): Sekundäres Gate: prüft im kleinen ROI um 'center_xy', ob dart-ähnliche\n- `_refine_tip_position()` (L636): Verfeinert die Impact-Position auf die Dartspitze.\n- `_is_same_position()` (L728): Check if two candidates are at same position\n- `_reset_tracking()` (L743): Reset temporal tracking\n- `get_confirmed_impacts()` (L748): Get all confirmed dart impacts\n- `clear_impacts()` (L752): Clear confirmed impacts (e.g., new game round)\n- `__post_init__()` (L772)\n- `__init__()` (L785)\n- `point_to_score()` (L791): Convert pixel coordinates to dartboard score.

## Intra-module calls (heuristic)
Canny, DartCandidate, DartDetectorConfig, DartImpact, FieldMapperConfig, GaussianBlur, HoughLinesP, _find_dart_shapes, _is_in_cooldown, _is_same_position, _preprocess_motion_mask, _refine_impact_in_roi, _refine_tip_position, _reset_tracking, abs, acos, append, arcLength, arctan2, astype, boundingRect, circle, clear, clip, column_stack, connectedComponentsWithStats, contourArea, convexHull, copy, count_nonzero, cvtColor, debug, degrees, deque, dict, dot, eigh, findContours, float, fromiter, get, getLogger, getStructuringElement, getattr, hasattr, hstack, hypot, imshow, info, int, len, line, linspace, locals, lower, max, mean, min, moments, morphologyEx, nonzero, norm, putText, range, replace, sort, sqrt, threshold
