# Module: `src\calibration\charuco_calibrator.py`
Hash: `e1c3685c3f95` · LOC: 4 · Main guard: false
...
## Classes
- `CharucoCalibrator` (L18): ChArUco-based calibration for camera intrinsics and dartboard pose.

## Functions
- `__init__()` (L25): Initialize ChArUco calibrator.\n- `detect_board()` (L106): Detect ChArUco board in frame.\n- `calibrate_camera_multi_frame()` (L184): Calibrate camera using multiple frames of ChArUco board.\n- `calibrate_camera_single_frame()` (L248): Quick calibration from single frame.\n- `get_dartboard_homography()` (L294): Calculate homography from ChArUco board to dartboard.\n- `undistort_image()` (L363): Remove lens distortion from image.\n- `draw_detected_board()` (L405): Draw detected ChArUco board on frame for visualization.\n- `save_calibration()` (L448): Save calibration to file.\n- `load_calibration()` (L484): Load calibration from file.

## Intra-module calls (heuristic)
ArucoDetector, CharucoBoard, DetectorParameters, Path, all, append, array, astype, calibrateCameraCharuco, circle, copy, cvtColor, debug, detectMarkers, detect_board, enumerate, error, estimatePoseCharucoBoard, exists, float, float32, getLogger, getOptimalNewCameraMatrix, getPerspectiveTransform, getPredefinedDictionary, info, int, len, load, matchImagePoints, mkdir, projectPoints, putText, range, ravel, reshape, savez, str, tuple, undistort, warning, zeros
