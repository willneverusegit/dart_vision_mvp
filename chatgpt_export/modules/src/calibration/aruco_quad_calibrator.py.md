# Module: `src\calibration\aruco_quad_calibrator.py`
Hash: `08389fa064d9` · LOC: 1 · Main guard: false
...
## Classes
- `ArucoQuadCalibrator` (L19)

## Functions
- `__init__()` (L20)\n- `detect()` (L42): Return marker corners and ids using new API if available.\n- `_centers_from_corners()` (L52): Compute centers (x,y) from list of 4x1x2 corner arrays.\n- `_order_points_tl_tr_br_bl()` (L62): Order 4 points as TL, TR, BR, BL (standard CV convention).\n- `estimate_homography_from_centers()` (L77): centers: (4,2) unsorted marker centers.\n- `_axis_aligned_box()` (L91): Return width_px, height_px of the axis-aligned box enclosing points TL,TR,BR,BL.\n- `estimate_scale_mm_per_px()` (L103): Compute mm/px using the average of horizontal and vertical scales.\n- `calibrate_from_frame()` (L117): Detect markers, compute homography from their centers, and (optionally) mm/px.\n- `draw_debug()` (L186)\n- `to_yaml_dict()` (L209)

## Intra-module calls (heuristic)
ArucoDetector, DetectorParameters, _axis_aligned_box, _centers_from_corners, _order_points_tl_tr_br_bl, append, argmax, argmin, argsort, array, asarray, astype, circle, contourArea, copy, cvtColor, detect, detectMarkers, diff, drawDetectedMarkers, enumerate, estimate_homography_from_centers, estimate_scale_mm_per_px, findHomography, flatten, float, getPredefinedDictionary, hasattr, int, inv, isinstance, len, line, list, max, mean, norm, perspectiveTransform, putText, range, ravel, reshape, sum, tolist, tuple, update
