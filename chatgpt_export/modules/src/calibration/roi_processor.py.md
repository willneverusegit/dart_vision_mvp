# Module: `src\calibration\roi_processor.py`
Hash: `3081a3847130` · LOC: 1 · Main guard: false
...
## Classes
- `ROIConfig` (L18): ROI processing configuration\n- `ROIProcessor` (L26): Region of Interest processor for dartboard isolation.

## Functions
- `__init__()` (L37)\n- `set_homography()` (L51): Set homography transformation from source points.\n- `set_homography_from_matrix()` (L100): Set homography directly from 3x3 matrix.\n- `warp_roi()` (L123): Apply perspective transform to extract ROI.\n- `_identity_fallback()` (L154): Fallback: Center-crop to ROI size without transformation.\n- `polar_unwrap()` (L177): Convert circular dartboard to linear polar coordinates.\n- `project_point_to_roi()` (L228): Project point from original frame to ROI coordinates.\n- `project_point_to_original()` (L265): Project point from ROI back to original frame coordinates.\n- `get_stats()` (L295): Get ROI processing statistics

## Intra-module calls (heuristic)
ROIConfig, _identity_fallback, array, copy, error, float32, getLogger, getPerspectiveTransform, info, int, inv, max, min, perspectiveTransform, reshape, resize, warning, warpPerspective, warpPolar
