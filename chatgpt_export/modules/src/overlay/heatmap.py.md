# Module: `src\overlay\heatmap.py`
Hash: `2fc57bf8e975` · LOC: 1 · Main guard: false
...
## Classes
- `HeatmapAccumulator` (L8): - Low-res grid (scale factor) for cheap updates

## Functions
- `__init__()` (L15)\n- `_maybe_decay()` (L49)\n- `add_hit()` (L63): Add hit in full-res coordinates.\n- `render_overlay()` (L80)\n- `export_png()` (L107): Save current heatmap (colorized) as PNG without blending.

## Intra-module calls (heuristic)
_maybe_decay, addWeighted, all, applyColorMap, arange, astype, clip, copy, exp, float, imwrite, int, max, merge, meshgrid, min, percentile, render_overlay, resize, round, time, zeros
