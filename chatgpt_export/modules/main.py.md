# Module: `main.py`
Hash: `67e47445a54b` · LOC: 1 · Main guard: true

## Imports
- `argparse`\n- `cv2`\n- `logging`\n- `math`\n- `numpy`\n- `os`\n- `sys`\n- `time`\n- `yaml`

## From-Imports
- `from pathlib import Path`\n- `from typing import Optional, Tuple, List`\n- `from collections import deque`\n- `from src.game.game import DemoGame, GameMode`\n- `from src.vision.dart_impact_detector import apply_detector_preset`\n- `from src.board import BoardConfig, BoardMapper, Calibration, draw_ring_circles, draw_sector_labels`\n- `from src.overlay.heatmap import HeatmapAccumulator`\n- `from src.analytics.polar_heatmap import PolarHeatmap`\n- `from src.analytics.stats_accumulator import StatsAccumulator`\n- `from src.capture import ThreadedCamera, CameraConfig, FPSCounter`\n- `from src.calibration.roi_processor import ROIProcessor, ROIConfig`\n- `from src.calibration.unified_calibrator import UnifiedCalibrator, CalibrationMethod`\n- `from src.vision import MotionDetector, MotionConfig, DartImpactDetector, DartDetectorConfig, FieldMapper, FieldMapperConfig`\n- `from src.utils.performance_profiler import PerformanceProfiler`\n- `from src.calibration.aruco_quad_calibrator import ArucoQuadCalibrator`\n- `from src.calibration.calib_io import save_calibration_yaml`\n- `from src.calibration.calib_io import load_calibration_yaml`\n- `from src.calibration.calib_io import load_yaml, load_unified_calibration, save_unified_calibration, compute_effective_H, mapper_calibration_from_unified`\n- `from src.board.config_models import UnifiedCalibration, Homography, Metrics, OverlayAdjust, ROIAdjust`\n- `from pathlib import Path`

## Classes
- `DartVisionApp` (L133)

## Functions
- `setup_logging()` (L62): Configure dual logging to console + file.\n- `__init__()` (L134)\n- `_ensure_roi_annulus_mask()` (L220): Build a static annulus mask in ROI coords (0..255) to ignore Netz/Hintergrund.\n- `_update_uc_from_calibrator()` (L235): Push calibration result from calibrator into UnifiedCalibration:\n- `_save_calibration_unified()` (L291): Persist calibration in unified format:\n- `_ratio_consistent()` (L359): Prüft, ob innere Ringe im erwarteten Verhältnis zu r_out auftauchen.\n- `_hough_refine_center()` (L379): Findet den äußeren Doppelring als Kreis und passt Overlay-Center & Scale an.\n- `score()` (L416)\n- `_hough_refine_rings()` (L457): Findet konzentrische Ringe via HoughCircles (im ROI-Panel).\n- `_auto_sector_rotation_from_edges()` (L527): Schätzt den Rotations-Offset (in Grad) für die Sektor-Ausrichtung anhand eines\n- `_hud_compute()` (L629)\n- `_hud_color()` (L644)\n- `_hud_draw()` (L653)\n- `_overlay_center_radius()` (L659): Gibt (cx, cy, r_base) zurück: Center inkl. Offsets + skalierten Doppel-Außenradius.\n- `_points_from_mapping()` (L666): Rechnet BoardMapper-Ergebnis in Punkte um.\n- `_draw_traffic_light()` (L684)\n- `st_b()` (L685)\n- `st_f()` (L687)\n- `st_e()` (L689)\n- `_sync_mapper()` (L709)\n- `_sync_mapper_from_unified()` (L716): Single source of truth → mapper Calibration and internal H_eff.\n- `build_roi_adjust_matrix()` (L725): Baut eine 3x3 Homographie, die um das ROI-Zentrum herum skaliert/rotiert und\n- `_effective_H()` (L768)\n- `_apply_effective_H_if_dirty()` (L774)\n- `_draw_help_overlay()` (L784): Zeichnet ein kompaktes Hilfe-Overlay unten rechts ins ROI-Bild (img).\n- `setup()` (L835)\n- `_calibration_ui()` (L1010)\n- `_metrics_for_hud()` (L1070)\n- `on_mouse()` (L1078)\n- `_apply_loaded_yaml()` (L1234)\n- `_apply_loaded_calibration()` (L1323): Backward/forward compatible loader:\n- `process_frame()` (L1435)\n- `_overlay_calib()` (L1524): Aktuelle Overlay-Geometrie (Single Source).\n- `create_visualization()` (L1539)\n- `run()` (L1744)\n- `_recalibrate_and_apply()` (L2038)\n- `cleanup()` (L2117)\n- `main()` (L2143)

## Intra-module calls (heuristic)
ArgumentParser, ArucoQuadCalibrator, BoardConfig, BoardMapper, Calibration, CameraConfig, Canny, DartDetectorConfig, DartImpactDetector, DartVisionApp, DemoGame, FPSCounter, FieldMapper, FieldMapperConfig, FileHandler, Formatter, GaussianBlur, HeatmapAccumulator, Homography, HoughCircles, Laplacian, Metrics, MotionConfig, MotionDetector, OverlayAdjust, Path, PerformanceProfiler, PolarHeatmap, ROIAdjust, ROIConfig, ROIProcessor, StatsAccumulator, StreamHandler, ThreadedCamera, UnifiedCalibration, UnifiedCalibrator, _apply_effective_H_if_dirty, _apply_loaded_calibration, _apply_loaded_yaml, _auto_sector_rotation_from_edges, _calibration_ui, _draw_help_overlay, _draw_traffic_light, _effective_H, _ensure_roi_annulus_mask, _homography_and_metrics, _hough_refine_rings, _hud_color, _hud_compute, _overlay_calib, _points_from_mapping, _ratio_consistent, _recalibrate_and_apply, _save_calibration_unified, _sync_mapper, _sync_mapper_from_unified, _update_uc_from_calibrator, abs, add, addHandler, addWeighted, add_argument, add_hit, add_mutually_exclusive_group, all, angle, append, apply, apply_detector_preset, apply_points, arctan2, around, array, asarray, astype, bitwise_and, bool, build_roi_adjust_matrix, calibrate_from_frame, circle, cleanup, clear, clear_impacts, clip, compute_effective_H, copy, cos, createCLAHE, create_visualization, cvtColor, debug, deg2rad, degrees, deque, destroyAllWindows, destroyWindow, detect_charuco, detect_dart, detect_motion, drawDetectedMarkers, drawFrameAxes, draw_debug, draw_ring_circles, draw_sector_labels, enumerate, error, estimate_pose_charuco, exists, exp, expanduser, export_csv, export_csv_dists, export_json, export_png, eye, float, float32, get, getLevelName, getLogger, getWindowProperty, get_confirmed_impacts, get_stats, getattr, hasattr, histogram, hypot, imshow, imwrite, info, int, isfinite, isinstance, len, line, load_calibration_yaml, lower, main, makedirs, mapper_calibration_from_unified, max, mean, medianBlur, min, namedWindow, nonzero, open, ord, overlay_panel, parse_args, print, process_frame, putText, range, ravel, read, rectangle, render_overlay, reset, reshape, resize, resolve, round, run, safe_load, save_calibration_yaml, save_unified_calibration, score_from_hit, selftest, set, setFormatter, setLevel, setMouseCallback, set_detector_params, set_homography_from_matrix, setup, setup_logging, sin, sleep, sorted, split, st_b, st_e, st_f, start, startswith, stop, str, switch_mode, theta0_effective, time, to_yaml_dict, toast, tolist, tune_params_for_resolution, tuple, uint16, update, upper, values, var, waitKey, waitKeyEx, warning, warp_roi, zeros
